      ...
      if (Command.cooldown) {
        if (cooldown.has(`slash-${Command.name}${interaction.user.id}`)) {
          return interaction.reply({
            content: `You're now in ${
              ms(`slash-${Command.name}${interaction.user.id}`) - Date.now()
            } cooldown!`,
          });
        }
        ...
        if (Command.userPerms || Command.botPerms) {
          if (
            ...
            const userPerms = new EmbedBuilder()
              .setDescription(
                `🚫 ${interaction.user}, You don't have \`${Command.userPerms}\` permissions to use this command!`
              )
              .setColor("Red");
        ...
        if (Command.userPerms || Command.botPerms) {
          ...
          if (
            ...
            const botPerms = new EmbedBuilder()
              .setDescription(
                `🚫 ${interaction.user}, I don't have \`${Command.botPerms}\` permissions to use this command!`
              )
              .setColor("Red");
            return interaction.reply({ embeds: [botPerms] });
          }
